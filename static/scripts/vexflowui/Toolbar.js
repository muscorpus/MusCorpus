Vex.UI.Toolbar = function(handler){
	...
	if(opts.canChangeNoteValue){
		...
		this.buttons.wildcard = noteValuesGroup.appendChild(this.createIcon("wildcard", "wildcard"));
		...
		this.buttons.wildcardRest = restValuesGroup.appendChild(this.createIcon("wildcard", "wildcard"));
...
Vex.UI.Toolbar.prototype.handleEvent = function(evt){
	...
	switch(evt.target.name){
		...
		case "wildcard":
		case "wildcardRest":
			this.handler.updateProvisoryType("rest");
...
Vex.UI.Toolbar.prototype.createTickableController = function(){
	...
	for(var i = 0; i<groups.length; i++){
		...
		icon.className += " vexui-icon-group"
	}
